...
class OLSRLinearIPLookup : public LinearIPLookup { public:

    const char *class_name() const	{ return "OLSRLinearIPLookup"; }
    const IPRoute * lookup_iproute(const IPAddress&) const;

    /// @TODO modify the insert so that it adds the nr of hops to the extra field
    void clear();

    void update(const IPAddress& dst, const IPAddress& gw, int port, int extra);

    typedef Vector<IPRoute>::iterator IPRouteTableIterator;
    IPRouteTableIterator begin() { return _t.begin(); };
    IPRouteTableIterator end() { return _t.end(); };
};
